# Walkthrough: Port Features + Retrieval Bugfix

## Changes Made

All changes in [ne4j-histo.py](file:///home/francisco/Escritorio/IA/histo-test/ne4j-histo.py). No changes needed in `server.py` (transparent).
...
**`_chunks()` method** (was line 2138, now ~2138-2175):
- **Before**: Naive 500-char slicing without overlap.
- **After**: Recursive splitting using separators `["\n\n", "\n", ". ", " ", ""]` with `chunk_size=800`, `chunk_overlap=150`.
- Preserves paragraph/sentence boundaries and adds context overlap between consecutive chunks.

### Feature 2 — Table Extraction with Groq Vision + Gemini Fallback

- `ExtractorImagenesPDF.__init__`: Added `llm` parameter.
- `ExtractorImagenesPDF._detectar_y_extraer_tabla()`: New async method that:
  1. Sends page image to **Groq Vision** (`llama-3.2-11b-vision-preview`).
  2. On failure, falls back to **Google Gemini** (`gemini-2.5-flash`).
  3. Returns Markdown table content or empty string.
- `Neo4jClient.crear_esquema()`: Added `Tabla` uniqueness constraint.
- `Neo4jClient.upsert_tabla()`: New method to insert `:Tabla` nodes with `contenido_md`, linked to `:PDF` and `:Pagina`.
- `indexar_en_neo4j()`: After each image upsert, calls table detection and creates `:Tabla` node if found.
- `inicializar_componentes()`: Injects LLM into extractor with `self.extractor_imagenes.llm = self.llm`.

### Feature 3 — SNOMED/FMA Ontology Normalization

- `ExtractorEntidades.extraer_de_texto()`: Now prompts for `{"nombre", "snomed_id", "fma_id"}` dicts instead of plain strings. Text input window increased to 1000 chars.
- `Neo4jClient.upsert_chunk()`: Handles both dict and string entities. Uses `coalesce($snomed_id, t.snomed_id)` to avoid overwriting existing IDs with null.
- Cross-relations section normalizes dicts to strings before creating `:CONTIENE` / `:TENIDA_CON` edges.

### Feature 4 — Hybrid Re-ranking (Near-Duplicate Boost)

- `Neo4jClient.busqueda_hibrida()`:
  1. **Near-duplicate detection**: After UNI and PLIP vector searches, identifies image IDs with score ≥ 0.95 in **both** models simultaneously.
  2. **Boost ×2.0**: In the `agregar()` fusion function, near-duplicate results get their weighted similarity doubled.
  3. **Logging**: Prints `🎯 NearDup=N` in the hybrid search summary when near-duplicates are found.

## Verification

- ✅ `py_compile` passes on both `ne4j-histo.py` and `server.py`.

## Next Steps

> [!IMPORTANT]
> You need to **re-index** to apply the new chunking, SNOMED IDs, and table extraction:
> ```bash
> python ne4j-histo.py --reindex --force
> ```
> Or via the server (which auto-indexes on startup if DB is empty — clear Neo4j first for a full re-index).
